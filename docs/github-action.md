...
The [`qain-diff` action](../.github/actions/qain-diff/README.md) watches the
qain snapshot baselines committed in your repo. When a pull request changes
one, it diffs the file against its merge-base version and posts **one sticky
comment** with the semantic, rule-attributed style diff per baseline —
**before / after / diff screenshots embedded** — updated in place on every
push, and **deleted again when the diff disappears** (e.g. the baseline
change is reverted).
...
Those two globs cover the default baseline locations of
[`@qain/vitest`](../packages/vitest/README.md) and
[`@qain/storybook`](../packages/storybook/README.md);
[`@qain/playwright`](../packages/playwright/README.md) baselines live in
`*.spec.ts-snapshots/`. Point `pattern` wherever yours are.

> **Do not add `on.pull_request.paths` filtering for the baseline files.**
> The cleanup half — deleting the comment once a baseline change is
> reverted — needs a run on the push where the file is *no longer* part of
> the PR's diff. The action exits in seconds when nothing matches.
...
captured with `qain snap --replay`, a Chrome/Chromium on the runner
(GitHub's `ubuntu-latest` ships one), and `contents: write` — the PNGs are
committed to the `assets-branch` (default `qain-diff-assets`, created on
first use) and embedded by immutable commit URL. Anything missing and the
action degrades to the text-only comment with a note.
...
| `screenshots` | `true` | Embed before/after/diff renders — a single render for new baselines (see above). |
...
- This repo [dogfoods the action](../.github/workflows/qain-baselines.yml)
  on its own `examples/baselines/*.qain.json`.
